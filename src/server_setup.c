...
int setup_listener(int port, int backlog) {
  ...
  // binds to port
  if (bind(listener_socket, (struct sockaddr *)&servAddr, sizeof(servAddr)) <
      0) {
    perror("Bind failed");
    exit(1);
  }
...
// function for client handling for connected sockets --> will be used as a
// thread
void accept_connections(int listener_socket) {
  ...
  while (1) {
    ...
    increment_client_count(); // uses locks to increment / decrement client
                              // count
...
void *write_handling() {
  while (1) {
    // printf("%d,%d,%s \n",current_msg.client_count,current_msg.socket_flag,
    // current_msg.msg_ptr);
...
int read_info(int socket, char *buf, int len) {
  ...
  while (total_received < sizeof(net_len)) {
    // Use sizeof(net_len) - total_received to ensure we only read the remaining
    // bytes
  ...
  if (msg_len >= (uint32_t)len) {
    fprintf(stderr,
            "Server buffer too small for incoming message. Disconnecting.\n");
  ...
  while (total_received < msg_len) {
    ...
    bytes_received =
        recv(socket, buf + total_received, msg_len - total_received, 0);
