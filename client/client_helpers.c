...
#define MAX_MESSAGE_SIZE                                                       \
  (1024 * 1024) // max size for reading terminal input (1MB)
volatile sig_atomic_t stop_client =
    0; // flag flipped when Ctrl+C hits, safe for signal handlers
...
void *read_message(void *client_socket_ptr) {
  ...
  while (stop_client != 1) {
    ...
    // Loop while the TCP stream hasn't sent 4 bytes, cause we need these 4
    // bytes to understand how long of a message we are going to need to buffer
    total_received = 0;
    while (total_received < sizeof(net_len)) {
      // read the amount of bytes recieved currently in our tcp_buffer (need to
      // understand this line better)
      bytes_received = recv(client_socket, ((char *)&net_len) + total_received,
                            4 - total_received, 0);
      // if we are getting no bytes from our TCP stream, or a -1 value,
      // something is wrong
      if (bytes_received <= 0) {
    ...
    while (total_received < msg_len) {
      // read the amount of message bytes recieved currently in our tcp_buffer
      // in the kernal
